...
- Cleanup/removal utilities:
  - `tools/media-cleanup-aliases`: Sourced by `.profile` from `~/tools` when present; defines aliases to hoist files from immediate-child `Vid*` folders, delete empty second-level `Videos` folders, run both operations in sequence, and recursively remove screen-artifact directories. The `delete-screen-dirs` loop runs entirely inside a subshell so its alias expansion is valid Bash and its `globstar` setting stays isolated.
